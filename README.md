...
When training multimodal models end-to-end, high-capacity projectors can suffer from representation drift. We apply a structural constraint regularizing the active cross-attention weights ({\text{after}}$) against a frozen pre-trained anchor pooler ({\text{before}}$):
