...
def _try_load_backend(backend_name):
    ...
    try:
        ...
        # Support the new TabPFN v2.6+ factory initialization
        if backend_name == "tabpfn":
            from tabpfn.constants import ModelVersion
            class V26WrapperClassifier:
                def __init__(self, n_estimators=1, device="cpu", **kwargs):
                    self.model = RawTabPFNClassifier.create_default_for_version(ModelVersion.V2_6, device=device)
                    # Forward attributes
            ...
            TabPFNClassifier = V26WrapperClassifier
        else:
