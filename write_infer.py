...
from dotpfn.scripts.infer import run_inference

if __name__ == "__main__":
    config = parse_args_and_config()
    run_inference(config)
'''
write_file('infer.py', infer_py_content)

infer_script_content = '''import os
import torch
import numpy as np
import pandas as pd
...
def run_inference(config: ConfigNode):
    ...
    query_df[out_cols].to(out_path, index=False)
    logger.info(f"Inference complete! Results saved to {out_path}")
'''
write_file('src/dotpfn/scripts/infer.py', infer_script_content)

yaml_content = '''# Config for Inference Pipeline
model_type: "multimodal"
device: "cuda"

tabpfn:
  version: "auto" # "auto", "tabpfn_extensions", "tabpfn", "tabpfn_v1"
  n_estimators: 1

data:
  support_csv_path: "/path/to/support_data.csv"
  query_csv_path: "/path/to/new_patients.csv"
  patient_id_col: "ID"
  target_label: "adherence_1yr"
  feature_cols:
    - 'SEX'
    - 'AGE'
    - 'Height'
    ...
    - 'REM_Episodes'
    - 'TST'
    - 'Total_Stage_N1_ratio'
    - 'Total_Stage_N2_ratio'
    - 'Total_Stage_N3_ratio'
    - 'Total_Stage_R_ratio'
    - 'WASO'
    - 'Sleep_latency'
    - 'REM_sleep_latency'
...
inference:
  model_weights_path: "/path/to/best_model_label_adherence_1yr_fold_0.pt"
...
model:
  ...
  dropout: 0.2
'''
write_file('config/infer.yaml', yaml_content)
